...
import { formatSuburbList, TUTORING_SUBURBS } from "../data/locations";
...
function ServiceDetail({ service, compact = false }: { service: ServiceItem; compact?: boolean }) {
  return (
    <div>
      {!compact && (
        <div className="rounded-2xl" style={{ height: "106px", width: "100%", background: service.gradient }} aria-hidden="true" />
      )}
      <div className={compact ? "" : "pt-5"}>
        ...
        {service.subjectGroups && (
          <div className="mt-3 space-y-3">
            {service.subjectGroups.map((group) => (
              <div key={group.label}>
                <p className="mb-1 font-sans text-[10px] font-semibold uppercase tracking-widest text-[#8EAF8A]">{group.label}</p>
        ...
        <a href="#contact" className="mt-5 inline-flex items-center gap-1.5 text-sm font-medium text-[#F0744A] transition-opacity hover:opacity-60">
          Book a free class →
        </a>
      </div>
    </div>
...
function MobileAccordion({ openIdx, onToggle }: { openIdx: number | null; onToggle: (i: number) => void }) {
  return (
    <div className="flex flex-col gap-1 pt-2">
      {SERVICES.map((service, i) => (
        <div key={service.title}>
          ...
          >
            <span className={`insight-heading transition-all duration-200 ${openIdx === i ? "text-2xl font-medium" : "text-xl font-normal"}`}>
              {service.title}
            </span>
            <span className="shrink-0 self-center">
              {openIdx === i ? <ChevronUp size={14} className="text-[#1A1615]" /> : <ChevronDown size={14} className="text-[#B5B0AA]" />}
            </span>
          </button>
          {openIdx === i && (
      ...
      <div className="hidden min-[480px]:flex gap-4 sm:gap-8 md:gap-14 lg:gap-20 xl:gap-28">
        <div className="flex flex-col gap-1 pt-2 flex-1 min-w-[250px]">
          {SERVICES.map((service, i) => (
            ...
            >
              <span className={`insight-heading transition-all duration-200 ${activeIdx === i ? "text-2xl font-medium" : "text-xl font-normal"}`}>
                {service.title}
              </span>
              <span className={`text-[9px] font-medium tracking-widest shrink-0 transition-colors duration-200 ${activeIdx === i ? "text-[#1A1615]" : "text-[#C8C3BE]"}`}>
                {padIndex(i)}
              </span>
            </button>
          ))}
        ...
        <div className="w-[40%] min-w-[250px] max-w-[400px] shrink-0">
          <div key={activeIdx} className="service-panel-animate">
            <ServiceDetail service={SERVICES[activeIdx]} />
          </div>
        </div>
      </div>
      <div className="min-[480px]:hidden">
        <MobileAccordion openIdx={openIdx} onToggle={(i) => setOpenIdx((p) => (p === i ? null : i))} />
      </div>
    </>
  );
}
